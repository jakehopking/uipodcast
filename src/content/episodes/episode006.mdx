...
In this week's episode I talk with Chris Biscardi about the Jamstack, gatsbyJS and his own meta-framework called Toast, among many other interesting digressions :)
...
Got into the industry via ActionsSript, then moved over JS when Adobe killed it off. Worked at Docker, Dropbox and recently finished a contract with GatsbyJS.
...
Back to Toast. Basically it's Gatsby without Webpack. It supports MDX as a first-class citizen. The best implementation of MDX is currently that which is in Gatsby, he knows because Chris the chap who built it. The implementations in other meta-frameworks such as NuxtJS and NextJS are ok. The beauty with Toast is that because it uses ESM, you could for example have a fully featured reactJS app load in your example MDX file, and this would only load when the user hits that page (this complexity is obviously not added to a non-existent bundle) the browser simple imports the ES modules it requires and your react app would load right there inside your post. This is simple not possible in the other implementations that rely on Webpack.
...
> NOTE: OUTAGE - We lose connection here for a few mins. So please forgive the broken content.
> Also, note to self (which you can read): create some CSS styling for blockquotes and ordered lists!!

- **If you could change one aspect of UI design or development in your industry, what would it be and why?**
...
- **For someone joining or is new to your industry and is feeling overwhelmed, what advice would you give them?**
  - The most important thing to remember is that not everything you are looking at: not everyone knows all of them. It's easy to get lost on the aggregator sites such as Twitter, and forget that everyone is on a different spike in knowledge. You need to remember that there will always be more to learn than is possible in a lifetime. So make sure that what you are learning and practicing is something you actually enjoy. Otherwise you will be unhappy. Careers knowledge industries are fairly long.
- **What's a tool, app or package that you use which you believe deserves more attention than it currently has?**
  - A CSS library called Knopf that uses CSS variable to completely configure your buttons.
